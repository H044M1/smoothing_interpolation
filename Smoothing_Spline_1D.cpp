...
double Smoothing_Spline_1D::Basis_Function(int Number, const double& Ksi) const{
	...
	default: { throw runtime_error("Error in the basis function number..."); break; }
	}
}
...
double Smoothing_Spline_1D::Der_Basis_Function(int Number, const double& Ksi) const{
	...
	default: { throw runtime_error("Error in the basis function derivative number..."); break; }
	}
}

void Smoothing_Spline_1D::Update_Spline(const vector<Point>& Points,
	const vector<double>& F_Value){
	this->Points.clear();
	for (auto& elem : Points) this->Points.push_back(elem);
	int Num_Segments = static_cast<int>(Points.size()) - 1;
	alpha.resize(Num_Segments + 1);
	vector <double> a, b, c;
	a.resize(Num_Segments + 1); b.resize(Num_Segments + 1); c.resize(Num_Segments + 1);
	function<void(size_t Num_Segment, const Point& P, const double& F_Val, const double& w)>
